...
def main():
    ...
    portfolio_manager.portfolio[['modified_duration',
                                 'dv01',
                                 'convexity']] = portfolio_manager.portfolio.apply(lambda x: bond_duration(x), axis=1)
    ...
    # Solution to 3.1
    logging.info("Solution to 3.1")
    var_analytics = reng.VaR(portfolio_manager.portfolio_pnl)

    for vp in config['MODEL']['n_day_var_m_percentile'].split("|"):
        n_day = int(vp.split("^")[0])
        m_percentile = int(vp.split("^")[1])
        pnl_vector = var_analytics.calculate_var('portfolio_daily_pnl', n_day, m_percentile/100)
        # print results
        logging.info("\n {}-Day VaR at {}%  {:.2f}".format(n_day,m_percentile, pnl_vector['var_value'].values[0]))
